...
for el in range(nel):
    ...
    K_uu, K_ww, f_u, f_w = kirchoff_plate_element(Ex[el, :], Ey[el, :], h_plate, Dbar, q=q_body, f=f_body)
...
def von_mises_plane_stress(sigma):
    ...
    s11, s22, s12 = sigma
    sigma_vm = np.sqrt(s11**2 - s11 * s22 + s22**2 + 3 * s12**2)
    return sigma_vm
...
for el in range(nel):
    ...
    K_uu_e, _, _, _ = kirchoff_plate_element(ex, ey, h_plate, Dbar, q=0, f=np.array([[0], [0]]))
